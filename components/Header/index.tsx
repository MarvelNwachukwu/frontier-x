...
export const Header =  () => {
  ...
  return(
    <header className='bg-[#0D0D0D] h-max min-h-[96px] flex gap-4 justify-between px-[1rem] md:px-[38px] items-center flex-wrap lg:flex-nowrap pb-4 lg:pb-0'>
      ...
      <form action='#' className={`grow px-4 items-center ${showMenuContent ? 'hidden' : 'flex'}`}>
      ...
      <div className={`CTA gap-[53px] grow h-12 ${showMenuContent ? 'flex' : 'hidden'} items-center justify-end pr-8 text-white`}>
        <a href='#'>
          Lorem
        </a>
        <a href='#'>
      ...
      <div className={`menu flex lg:hidden text-white h-8 w-8 ${showMenuContent ? "bg-[url('/close.png')]" : "bg-[url('/menu.png')]"} bg-cover bg-no-repeat cursor-pointer`} onClick={() => { updateShowMenuContent(!showMenuContent)}}/>
    </header>
  )
}